                        ...
                        <div className="bg-white rounded shadow-2xl border border-slate-300 overflow-hidden">
                            <div className="bg-slate-50 border-b p-4 flex justify-between items-center">
                                <h2 className="w-full text-center text-sm font-black text-slate-700 uppercase tracking-widest">
                                    {editingSeries ? 'Editar Série Fiscal' : 'Adicionar Nova Série Fiscal'}
                                </h2>
                            </div>
                            <div className="p-8 space-y-6 bg-white">
                                <div>
                                    <label className="text-xs font-bold text-slate-700 block mb-1">Nome da Série (Descritivo) *</label>
                                    <input
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="text-xs font-bold text-slate-700 block mb-1">Código Fiscal *</label>
                                        <input
                                    ...
                                    <div>
                                        <label className="text-xs font-bold text-slate-700 block mb-1">Exercício Fiscal</label>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label className="text-xs font-bold text-slate-700 block mb-1">Estado de Operação</label>
                                        <select
                                            className="w-full p-3 border border-slate-300 rounded-xl bg-white shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-medium text-slate-700 cursor-pointer"
                                ...
                                <div className="border-t border-slate-200 pt-4">
                                    ...
                                    <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 max-h-40 overflow-y-auto space-y-2">
                                        {localUsers.map(u => (
                                            <label key={u.id} className={`flex items-center justify-between p-2 rounded-lg cursor-pointer transition-all ${newSeriesData.allowedUserIds?.includes(u.id) ? 'bg-blue-50 border border-blue-200' : 'hover:bg-slate-100'}`}>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="text-xs font-bold text-slate-700 block mb-1">Dados Bancários</label>
                                        <textarea
                                            className="w-full p-3 border border-slate-300 rounded-xl bg-white/80 shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-medium text-slate-700 resize-none h-24"
                                            value={String(newSeriesData.bankDetails || '')}
                                            onChange={e => setNewSeriesData({ ...newSeriesData, bankDetails: e.target.value })}
                                            placeholder="IBAN BFA: AO06 0001..."
                                        />
                                    </div>
                                    <div>
                                        <label className="text-xs font-bold text-slate-700 block mb-1">Texto de Rodapé</label>
                                        <textarea
                                            className="w-full p-3 border border-slate-300 rounded-xl bg-white/80 shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-medium text-slate-700 resize-none h-24"
                                ...
                                <div className="pt-6">
                                    <button onClick={handleSaveSeries} disabled={isLoading} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold text-lg py-3 rounded-xl shadow-lg transition transform active:scale-95 flex items-center justify-center gap-3">
                                        {isLoading ? <Loader2 className="animate-spin" size={20} /> : 'Registar'}
                                    </button>
                                    <button onClick={() => setIsSeriesModalOpen(false)} className="w-full text-slate-400 font-bold text-[10px] uppercase mt-4 hover:text-slate-600 transition tracking-widest">Cancelar</button>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            {isBankModalOpen && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[110] flex items-center justify-center p-4 animate-in fade-in duration-300">
                    <div className="max-w-xl w-full animate-in zoom-in-95 duration-300">
                        <div className="bg-white rounded shadow-2xl border border-slate-300 overflow-hidden">
                            <div className="bg-slate-50 border-b p-4 flex justify-between items-center">
                                <h2 className="w-full text-center text-sm font-black text-slate-700 uppercase tracking-widest">Registar Novo Banco</h2>
                            </div>
                            <div className="p-8 space-y-6 bg-white">
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="text-xs font-bold text-slate-700 block mb-1">Sigla do Banco *</label>
                                        <input className="w-full p-3 border border-slate-300 rounded-xl bg-white/80 shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-bold text-slate-700 uppercase" value={newBank.sigla} onChange={e => setNewBank({ ...newBank, sigla: e.target.value })} placeholder="Ex: BAI, BFA, BIC" />
                                    </div>
                                    <div>
                                        <label className="text-xs font-bold text-slate-700 block mb-1">Código SWIFT / BIC</label>
                                        <input className="w-full p-3 border border-slate-300 rounded-xl bg-white/80 shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-medium text-slate-700 uppercase" value={newBank.swift} onChange={e => setNewBank({ ...newBank, swift: e.target.value })} placeholder="Ex: BAIAO..." />
                                    </div>
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-slate-700 block mb-1">Número de Conta</label>
                                    <input className="w-full p-3 border border-slate-300 rounded-xl bg-white/80 shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-medium text-slate-700" value={newBank.conta} onChange={e => setNewBank({ ...newBank, conta: e.target.value })} placeholder="Introduza o número de conta oficial" />
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-slate-700 block mb-1">NIB (21 Dígitos)</label>
                                    <input className="w-full p-3 border border-slate-300 rounded-xl bg-white/80 shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-medium text-slate-700" value={newBank.nib} onChange={e => setNewBank({ ...newBank, nib: e.target.value })} placeholder="00010000..." />
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-slate-700 block mb-1">IBAN Angolano (AO06) *</label>
                                    <input className="w-full p-3 border border-slate-300 rounded-xl bg-white/80 shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-bold text-slate-700" value={newBank.iban} onChange={e => setNewBank({ ...newBank, iban: e.target.value })} placeholder="AO06 0001 0000..." />
                                </div>
                                <div className="pt-6">
                                    <button onClick={handleSaveBank} disabled={isLoading} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold text-lg py-3 rounded-xl shadow-lg transition transform active:scale-95 flex items-center justify-center gap-3">
                                        {isLoading ? <Loader2 className="animate-spin" size={20} /> : 'Registar'}
                                    </button>
                                    <button onClick={() => setIsBankModalOpen(false)} className="w-full text-slate-400 font-bold text-[10px] uppercase mt-4 hover:text-slate-600 transition tracking-widest">Cancelar</button>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            {isUserModalOpen && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[110] flex items-center justify-center p-4 animate-in fade-in duration-300">
                    <div className="max-w-2xl w-full animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-auto">
                        <div className="bg-white rounded shadow-2xl border border-slate-300 overflow-hidden">
                            <div className="bg-slate-50 border-b p-4 flex justify-between items-center">
                                <h2 className="w-full text-center text-sm font-black text-slate-700 uppercase tracking-widest">Cadastro de Utilizador</h2>
                            </div>
                            <div className="p-8 space-y-6 bg-white">
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="text-xs font-bold text-slate-700 block mb-1">Nome Completo *</label>
                                        <input className="w-full p-3 border border-slate-300 rounded-xl bg-white/80 shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-medium text-slate-700" value={String(newUser.name || '')} onChange={e => setNewUser({ ...newUser, name: e.target.value })} placeholder="Ex: João Manuel Silva" />
                                    </div>
                                    <div>
                                        <label className="text-xs font-bold text-slate-700 block mb-1">Utilizador (Username) *</label>
                                        <input className="w-full p-3 border border-slate-300 rounded-xl bg-white/80 shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-bold text-slate-700" value={String(newUser.username || '')} onChange={e => setNewUser({ ...newUser, username: e.target.value })} placeholder="username" />
                                    </div>
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="text-xs font-bold text-slate-700 block mb-1">Email Institucional *</label>
                                        <input type="email" className="w-full p-3 border border-slate-300 rounded-xl bg-white/80 shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-medium text-slate-700" value={String(newUser.email || '')} onChange={e => setNewUser({ ...newUser, email: e.target.value })} placeholder="email@empresa.ao" />
                                    </div>
                                    <div>
                                        <label className="text-xs font-bold text-slate-700 block mb-1">Palavra-passe *</label>
                                        <input type="password" className="w-full p-3 border border-slate-300 rounded-xl bg-white/80 shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-medium text-slate-700" value={String(newUser.password || '')} onChange={e => setNewUser({ ...newUser, password: e.target.value })} placeholder="••••••••" />
                                    </div>
                                </div>
                                <div className="grid grid-cols-3 gap-4">
                                    <div>
                                        <label className="text-xs font-bold text-slate-700 block mb-1">Contacto</label>
                                        <input className="w-full p-3 border border-slate-300 rounded-xl bg-white/80 shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-medium text-slate-700" value={String(newUser.phone || '')} onChange={e => setNewUser({ ...newUser, phone: e.target.value })} placeholder="+244 9..." />
                                    </div>
                                    <div>
                                        <label className="text-xs font-bold text-slate-700 block mb-1">Validade Acesso</label>
                                        <input type="date" className="w-full p-3 border border-slate-300 rounded-xl bg-white/80 shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-medium text-slate-700" value={String(newUser.accessValidity || '')} onChange={e => setNewUser({ ...newUser, accessValidity: e.target.value })} />
                                    </div>
                                    <div>
                                        <label className="text-xs font-bold text-slate-700 block mb-1">Perfil</label>
                                        <select className="w-full p-3 border border-slate-300 rounded-xl bg-white shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-medium text-slate-700 cursor-pointer" value={String(newUser.role || 'OPERATOR')} onChange={e => setNewUser({ ...newUser, role: e.target.value as any })}>
                                        ...
                                        {permissionGroups.map(group => (
                                            <div key={group.label} className="space-y-2">
                                                ...
                                                <div className="grid grid-cols-2 gap-2">
                                                    {group.perms.map(p => (
                                                        <label key={p.id} className={`flex items-center gap-2 p-2 rounded-lg cursor-pointer transition-all ${newUser.permissions?.includes(p.id as ViewState) ? 'bg-blue-50 border border-blue-200' : 'hover:bg-slate-100'}`}>
                                                            <input
                                                                type="checkbox"
                                                                className="w-4 h-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                                ...
                                <div className="pt-6">
                                    <button onClick={handleSaveUser} disabled={isLoading} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold text-lg py-3 rounded-xl shadow-lg transition transform active:scale-95 flex items-center justify-center gap-3 disabled:opacity-50">
                                        {isLoading ? <Loader2 className="animate-spin" size={20} /> : 'Registar'}
                                    </button>
                                    <button onClick={() => setIsUserModalOpen(false)} className="w-full text-slate-400 font-bold text-[10px] uppercase mt-4 hover:text-slate-600 transition tracking-widest">Cancelar</button>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            {isMetricModalOpen && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[110] flex items-center justify-center p-4 animate-in fade-in duration-300">
                    <div className="max-w-xl w-full animate-in zoom-in-95 duration-300">
                        <div className="bg-white rounded shadow-2xl border border-slate-300 overflow-hidden">
                            <div className="bg-slate-50 border-b p-4 flex justify-between items-center">
                                <h2 className="w-full text-center text-sm font-black text-slate-700 uppercase tracking-widest">Registar Unidade de Medida</h2>
                            </div>
                            <div className="p-8 space-y-6 bg-white">
                                <div>
                                    <label className="text-xs font-bold text-slate-700 block mb-1">Sigla / Abreviatura *</label>
                                    <input className="w-full p-3 border border-slate-300 rounded-xl bg-white/80 shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-bold text-slate-700 uppercase" value={newMetric.sigla} onChange={e => setNewMetric({ ...newMetric, sigla: e.target.value })} placeholder="Ex: kg, un, l, m2" />
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-slate-700 block mb-1">Descrição por Extenso *</label>
                                    <input className="w-full p-3 border border-slate-300 rounded-xl bg-white/80 shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-medium text-slate-700" value={newMetric.nome} onChange={e => setNewMetric({ ...newMetric, nome: e.target.value })} placeholder="Ex: Kilogramas, Unidades" />
                                </div>
                                <div className="pt-6">
                                    <button onClick={handleSaveMetric} disabled={isLoading} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold text-lg py-3 rounded-xl shadow-lg transition transform active:scale-95 flex items-center justify-center gap-3">
                                        {isLoading ? <Loader2 className="animate-spin" size={20} /> : 'Registar'}
                                    </button>
                                    <button onClick={() => setIsMetricModalOpen(false)} className="w-full text-slate-400 font-bold text-[10px] uppercase mt-4 hover:text-slate-600 transition tracking-widest">Cancelar</button>
                                </div>
                            </div>
                        </div>
                    </div>
