...
const TaxDocsManager: React.FC = () => {
  ...
  async function fetchTaxDocs() {
    ...
    try {
      ...
      if (data) {
        // Mapear dados da tabela pagamento_imposto para TaxDocument
        const mapped: TaxDocument[] = data.map(d => ({
          id: d.id,
          dateDoc: d.data_pagamento,
          dateContab: d.data_pagamento, // Fallback
          name: d.tipo_imposto || 'Imposto Indefinido',
          description: `Período: ${d.periodo}`,
          reference: d.periodo || 'S/N',
          amountPaid: Number(d.valor || 0),
          observations: d.estado || '',
          fileUrl: undefined, // Tabela não suporta arquivos
          fileName: undefined,
          occurrences: [] // Tabela não suporta ocorrências
        }));
        setDocuments(mapped);
      }
    } catch (err: any) {
  ...
  const handleCreate = () => {
    ...
    setFormData({
      name: 'IVA',
      description: '',
      reference: new Date().toISOString().slice(0, 7), // YYYY-MM
      amountPaid: 0,
      observations: '',
      dateDoc: new Date().toISOString().split('T')[0],
      dateContab: new Date().toISOString().split('T')[0],
  ...
  const handleSaveDocument = async () => {
    if (!formData.name || !formData.amountPaid) return alert("Preencha o tipo de imposto e o valor.");

    setIsLoading(true);
    const docId = editingDoc?.id || generateId();
    // UUID check skipped as we use Supabase generation if needed, or pass ID.
    // However, pagamento_imposto uses UUID primary key.

    // Payload compatível com a tabela pagamento_imposto
    const payload = {
      tipo_imposto: formData.name,
      valor: formData.amountPaid,
      periodo: formData.reference, // Usamos referência como período
      data_pagamento: formData.dateDoc,
      estado: 'Pago'
    };

    try {
      // Se for edição, precisamos do ID. Se for novo, deixamos criar.
      // Upsert pode precisar do ID se existir.

      let query = supabase.from('pagamento_imposto');
      let result;

      if (editingDoc?.id) {
        result = await query.update(payload).eq('id', editingDoc.id);
      } else {
      ...
      setShowModal(false);
      setEditingDoc(null);
      alert("Pagamento de imposto salvo com sucesso na Cloud!");
    } catch (err: any) {
      console.error(err);
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Deseja apagar este registo de imposto da Cloud?")) {
      setIsLoading(true);
      try {
        const { error } = await supabase.from('pagamento_imposto').delete().eq('id', id);
        if (error) throw error;
        await fetchTaxDocs();
