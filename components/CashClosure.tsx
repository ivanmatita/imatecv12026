...
import { formatCurrency, generateId, ensureUUID } from '../utils';
...
const CashClosure: React.FC<CashClosureProps> = ({ registers, invoices, movements, onSaveClosure, onGoBack, currentUser, currentUserId, companyId }) => {
    ...
    const handleClose = async () => {
        ...
        // ID generation already fixed in utils, but applying ensureUUID on companyId/registerId for safety
        ...
        try {
            const syncPayload = {
                id: closure.id,
                caixa_id: ensureUUID(selectedRegisterId), // Critical Fix: ensures UUID format
                data_fecho: timestamp.split('T')[0],
                ...
                empresa_id: companyId ? ensureUUID(companyId) : '00000000-0000-0000-0000-000000000001'
            };

            // ... rest of function ...

            const { error } = await supabase.from('fechos_caixa').insert(syncPayload).select();
