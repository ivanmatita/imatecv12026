...
import { Printer, Download, Calculator, Briefcase, ChevronDown, Save, ArrowRight, Table, FileText, HelpCircle, AlertCircle, CreditCard, Plus } from 'lucide-react';
import { listarImpostos, pagarImposto } from '../services/supabaseClient';
...
const TaxManager: React.FC<TaxManagerProps> = ({ invoices = [], company, purchases = [], payroll = [], stockMovements = [] }) => {
    ...
    // State for Payments Logic
    const [payments, setPayments] = useState<any[]>([]);
    const [newPayment, setNewPayment] = useState({ tipo_imposto: 'IVA', valor: 0, periodo: '', data_pagamento: new Date().toISOString().split('T')[0] });
    ...
    async function loadPayments() {
        ...
        try {
            const data = await listarImpostos();
    ...
    async function handlePayTax() {
        if (!newPayment.valor || !newPayment.periodo) return alert("Preencha valor e período.");
        setIsPaymentLoading(true);
        try {
            await pagarImposto(newPayment);
            await loadPayments();
            setNewPayment({ ...newPayment, valor: 0, periodo: '' });
    ...
    const renderStampDuty = () => {
        ...
        return (
            <div className="bg-white p-8 max-w-[1400px] mx-auto shadow-2xl border border-slate-300 font-sans animate-in zoom-in-95">
                {/* Header */}
    ...
    return (
        <div className="p-6 bg-slate-50 min-h-screen pb-20 animate-in fade-in font-sans">
            {/* Teramind Header */}
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-gradient-to-r from-orange-500 to-amber-600 p-6 rounded-2xl shadow-lg border border-orange-500/20 text-white mb-6 print:hidden">
                <div>
                    <h1 className="text-2xl font-light flex items-center gap-2 tracking-tight">
                        <Calculator className="text-white/80" /> Gestão de Impostos
                    </h1>
                    <p className="text-xs text-orange-100 font-bold uppercase tracking-widest mt-1">
                        Mapas fiscais e apuramentos oficiais
                    </p>
                </div>

                <div className="flex items-center gap-2 overflow-x-auto max-w-full p-1 bg-white/10 rounded-xl backdrop-blur-sm">
                    {['ANUAL', 'SELO', 'PAGAMENTOS'].map(t => (
                        <button
                            key={t}
                            onClick={() => setActiveTab(t)}
                            className={`px-4 py-2 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all border ${activeTab === t ? 'bg-white text-orange-600 border-white shadow-md' : 'bg-transparent text-white border-transparent hover:bg-white/10'}`}
                        >
                            {t === 'ANUAL' ? 'Modelo 1' : t === 'SELO' ? 'Imposto Selo' : 'Pagamentos'}
                        </button>
                    ))}
                </div>
            </div>
            ...
            {activeTab === 'PAGAMENTOS' && (
                <div className="bg-white p-6 rounded-lg shadow border border-slate-200 animate-in fade-in">
                    <h2 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2"><CreditCard /> Registo de Pagamentos Fiscais</h2>

                    <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6 bg-slate-50 p-4 rounded border border-slate-200">
                        <div>
                            <label className="block text-xs font-bold uppercase mb-1">Tipo de Imposto</label>
                            <select
                                className="w-full p-2 border rounded"
                                value={newPayment.tipo_imposto}
                                onChange={e => setNewPayment({ ...newPayment, tipo_imposto: e.target.value })}
                            >
                                <option value="IVA">IVA</option>
                                <option value="IRT">IRT</option>
                                <option value="INSS">INSS</option>
                                <option value="SELO">Imposto de Selo</option>
                                <option value="INDUSTRIAL">Imp. Industrial</option>
                            </select>
                        </div>
                        <div>
                            <label className="block text-xs font-bold uppercase mb-1">Período (Ex: 2024-01)</label>
                            <input
                                className="w-full p-2 border rounded"
                                type="month"
                                value={newPayment.periodo}
                                onChange={e => setNewPayment({ ...newPayment, periodo: e.target.value })}
                            />
                        </div>
                        <div>
                        ...
                        <div>
                            <label className="block text-xs font-bold uppercase mb-1">Valor (Kz)</label>
                            <input
                                className="w-full p-2 border rounded"
                                type="number"
                                value={newPayment.valor}
                                onChange={e => setNewPayment({ ...newPayment, valor: parseFloat(e.target.value) })}
                            />
                        </div>
                        <div className="flex items-end">
                            <button
                                ...
                                className="w-full bg-green-600 text-white p-2 rounded font-bold uppercase text-xs hover:bg-green-700 flex items-center justify-center gap-2"
                            >
                                {isPaymentLoading ? '...' : <><Plus size={14} /> Registar Pagamento</>}
                            </button>
                        </div>
                    </div>
                            ...
                            <tbody>
                                {payments.map((p, i) => (
                                    <tr key={i} className="border-b hover:bg-slate-50">
                                        <td className="p-3 font-mono">{formatDate(p.data_pagamento)}</td>
                                        <td className="p-3 font-bold">{p.tipo_imposto}</td>
                                        <td className="p-3">{p.periodo}</td>
                                        <td className="p-3 text-right font-mono font-bold">{formatCurrency(p.valor || 0)}</td>
                                        <td className="p-3 text-center"><span className="bg-green-100 text-green-700 px-2 py-1 rounded text-xs font-bold">{p.estado || 'Pago'}</span></td>
                                    </tr>
                                ))}
                                {payments.length === 0 && (
                                    <tr><td colSpan={5} className="p-8 text-center text-slate-400 italic">Sem pagamentos registados</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
