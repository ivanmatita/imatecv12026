...
const SecretariaForm: React.FC<SecretariaFormProps> = ({ document: existingDoc, onSave, onCancel, series }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            // Fix: Use 'secretaria' table with correct column names matching Step 259 schema + Step 262 migration
            const { error } = await supabase
                .from('secretaria')
                .upsert({
                    id: docId,
                    tipo_documento: docToSave.type,
                    numero: docToSave.number,
                    data_doc: docToSave.date,
                    destinatario: docToSave.destinatarioNome,
                    titulo: docToSave.assunto,
                    conteudo: docToSave.corpo,
