                    ...
                    <div className="max-w-2xl w-full animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-auto">
                        <div className="bg-white rounded shadow-2xl border border-slate-300 overflow-hidden">
                            <div className="bg-slate-50 border-b p-4 flex justify-between items-center">
                                <h2 className="w-full text-center text-sm font-black text-slate-700 uppercase tracking-widest">Lançar Entrada de Stock Manual</h2>
                            </div>
                            <div className="p-8 space-y-6 bg-white">
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="text-xs font-bold text-slate-700 block mb-1">Armazém de Destino *</label>
                                        <select
                                    ...
                                    <div>
                                        <label className="text-xs font-bold text-slate-700 block mb-1">Adicionar Produto</label>
                                        <select
                                            className="w-full p-3 border border-slate-300 rounded-xl bg-white shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-medium text-slate-700 cursor-pointer"
                                    ...
                                    <table className="w-full text-left text-xs">
                                        <thead className="bg-slate-100 text-slate-600 font-bold uppercase text-[10px]">
                                            <tr>
                                                ...
                                                <th className="p-3 text-center">Tipologia</th>
                                        ...
                                        <tbody className="divide-y divide-slate-100">
                                            {items.map((item, idx) => (
                                                <tr key={idx}>
                                                    ...
                                                    <td className="p-3">
                                                        <select
                                                            className="w-full p-2 border border-slate-200 rounded-lg text-center font-medium text-xs"
                                                            value={item.itemType}
                                                            onChange={e => {
                                            ...
                                            {items.length === 0 && (
                                                <tr><td colSpan={5} className="p-8 text-center text-slate-400 italic">Nenhum item adicionado à lista.</td></tr>
                                            )}
                                        </tbody>
                                    </table>
                                </div>
                                ...
                                <div className="pt-6">
                                    <button onClick={handleEmitirEntrada} disabled={isSaving || !warehouseId || items.length === 0} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold text-lg py-3 rounded-xl shadow-lg transition transform active:scale-95 flex items-center justify-center gap-3 disabled:opacity-50">
                                        {isSaving ? <Loader2 className="animate-spin" size={20} /> : 'Confirmar Entrada'}
                                    </button>
                                    <button onClick={() => setShowProductModal(false)} className="w-full text-slate-400 font-bold text-[10px] uppercase mt-4 hover:text-slate-600 transition tracking-widest">Cancelar</button>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            {showWarehouseModal && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-in fade-in duration-300">
                    <div className="max-w-xl w-full animate-in zoom-in-95 duration-300">
                        <div className="bg-white rounded shadow-2xl border border-slate-300 overflow-hidden">
                            <div className="bg-slate-50 border-b p-4 flex justify-between items-center">
                                <h2 className="w-full text-center text-sm font-black text-slate-700 uppercase tracking-widest">Registar Novo Armazém</h2>
                            </div>
                            <div className="p-8 space-y-6 bg-white">
                                <div>
                                    <label className="text-xs font-bold text-slate-700 block mb-1">Nome do Armazém *</label>
                                    <input className="w-full p-3 border border-slate-300 rounded-xl bg-white/80 shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-bold text-slate-700 uppercase" value={newWarehouse.name} onChange={e => setNewWarehouse({ ...newWarehouse, name: e.target.value })} placeholder="Ex: Armazém Central Viana" />
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-slate-700 block mb-1">Descrição do Armazém</label>
                                    <input className="w-full p-3 border border-slate-300 rounded-xl bg-white/80 shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-medium text-slate-700" value={newWarehouse.description} onChange={e => setNewWarehouse({ ...newWarehouse, description: e.target.value })} placeholder="Finalidade do Armazém (Ex: Stock Principal)" />
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-slate-700 block mb-1">Localização</label>
                                    <input className="w-full p-3 border border-slate-300 rounded-xl bg-white/80 shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-medium text-slate-700" value={newWarehouse.location} onChange={e => setNewWarehouse({ ...newWarehouse, location: e.target.value })} placeholder="Endereço Físico" />
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="text-xs font-bold text-slate-700 block mb-1">Responsável</label>
                                        <input className="w-full p-3 border border-slate-300 rounded-xl bg-white/80 shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-medium text-slate-700" value={newWarehouse.managerName} onChange={e => setNewWarehouse({ ...newWarehouse, managerName: e.target.value })} placeholder="Gestor do Armazém" />
                                    </div>
                                    <div>
                                        <label className="text-xs font-bold text-slate-700 block mb-1">Contacto</label>
                                        <input className="w-full p-3 border border-slate-300 rounded-xl bg-white/80 shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-medium text-slate-700" value={newWarehouse.contact} onChange={e => setNewWarehouse({ ...newWarehouse, contact: e.target.value })} placeholder="(+244) 9XX XXX XXX" />
                                    </div>
                                </div>
                                <div>
                                    <label className="text-xs font-bold text-slate-700 block mb-1">Observações</label>
                                    <textarea className="w-full p-3 border border-slate-300 rounded-xl bg-white/80 shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-medium text-slate-700 resize-none h-20" value={newWarehouse.observations} onChange={e => setNewWarehouse({ ...newWarehouse, observations: e.target.value })} placeholder="Notas adicionais..."></textarea>
                                </div>
                                <div className="pt-6">
                                    <button onClick={handleSaveWarehouse} disabled={isSaving} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold text-lg py-3 rounded-xl shadow-lg transition transform active:scale-95 flex items-center justify-center gap-3">
                                        {isSaving ? <RefreshCw className="animate-spin" size={20} /> : 'Registar'}
                                    </button>
                                    <button onClick={() => setShowWarehouseModal(false)} className="w-full text-slate-400 font-bold text-[10px] uppercase mt-4 hover:text-slate-600 transition tracking-widest">Cancelar</button>
                                </div>
                            </div>
                        </div>
                    </div>
