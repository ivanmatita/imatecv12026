...
const CashManager: React.FC<CashManagerProps> = ({ cashRegisters: propRegisters, onUpdateCashRegister, movements, onAddMovement, invoices, purchases }) => {
        ...
        try {
            ...
            if (regsError) {
                ...
                const mapped: CashRegister[] = regsData.map(r => ({
                    id: r.id,
                    name: r.titulo || r.nome || 'Caixa',
                    status: (r.status || 'CLOSED') as any,
                    operatorId: r.responsavel_id,
                    initialBalance: Number(r.saldo_inicial || r.saldo_abertura || 0),
                    balance: Number(r.saldo_atual || r.balance || 0),
                    notes: r.observacoes || r.descricao
                }));
    ...
    const handleSaveRegister = async () => {
        ...
        const payload = {
            nome: editingRegister.name,
            titulo: editingRegister.name,
            descricao: editingRegister.notes || '',
            status: editingRegister.status || 'CLOSED',
            saldo_inicial: Number(editingRegister.initialBalance || 0),
            saldo_atual: isNew ? Number(editingRegister.initialBalance || 0) : Number(editingRegister.balance || 0),
                    ...
                    <div className="max-w-xl w-full animate-in zoom-in-95 duration-300">
                        <div className="bg-white rounded shadow-2xl border border-slate-300 overflow-hidden">
                            <div className="bg-slate-50 border-b p-4 flex justify-between items-center">
                                <h2 className="w-full text-center text-sm font-black text-slate-700 uppercase tracking-widest">{editingRegister.id ? 'Editar Caixa' : 'Criar Novo Caixa'}</h2>
                            </div>
                            <div className="p-8 space-y-6 bg-white">
                                <div>
                                    <label className="text-xs font-bold text-slate-700 block mb-1">Título da Caixa *</label>
                                    <input
                                        className="w-full p-3 border border-slate-300 rounded-xl bg-white/80 shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-bold text-slate-700"
                                ...
                                <div>
                                    <label className="text-xs font-bold text-slate-700 block mb-1">Responsável da Caixa</label>
                                    <select
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label className="text-xs font-bold text-slate-700 block mb-1">Saldo de Abertura</label>
                                        <input
                                            type="number"
                                            className="w-full p-3 border border-slate-300 rounded-xl bg-white/80 shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-bold text-slate-700"
                                            placeholder="0.00"
                                ...
                                <div>
                                    ...
                                    <textarea
                                        className="w-full p-3 border border-slate-300 rounded-xl bg-white/80 shadow-inner outline-none focus:ring-1 focus:ring-blue-500 font-medium text-slate-700 resize-none h-20"
                                ...
                                <div className="pt-6">
                                    <button
                                        ...
                                        className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold text-lg py-3 rounded-xl shadow-lg transition transform active:scale-95 flex items-center justify-center gap-3 disabled:opacity-50"
                                    >
                                        {isLoadingCloud ? <RefreshCw className="animate-spin" size={20} /> : 'Registar'}
                                    </button>
                                    <button onClick={() => setShowRegisterModal(false)} className="w-full text-slate-400 font-bold text-[10px] uppercase mt-4 hover:text-slate-600 transition tracking-widest">Cancelar</button>
                                </div>
                            </div>
                        </div>
                    </div>
