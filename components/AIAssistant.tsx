...
const AIAssistant: React.FC<AIAssistantProps> = ({ invoices, purchases, clients }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const safeContent = typeof response === 'object' ? JSON.stringify(response) : String(response || 'Não recebi uma resposta.');
      setMessages(prev => [...prev, { role: 'bot', content: safeContent }]);
    } catch (error) {
