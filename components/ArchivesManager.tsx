...
const ArchivesManager: React.FC = () => {
  ...
  async function fetchArchives() {
    setIsLoading(true);
    try {
      // Use client function to list from STORAGE (as requested by user code)
      // But also fetch METADATA from DB to show rich info
      const { listarArquivos, supabase } = await import('../services/supabaseClient');
      ...
      // Fetch Storage Files
      const { data: storageData, error: storageError } = await listarArquivos();
      if (storageError) console.warn("Erro ao listar storage:", storageError);

      if (dbData) {
        const mapped: ArchiveDocument[] = dbData.map((d: any) => ({
          id: d.id,
          name: d.nome || 'Documento',
          type: d.tipo as any,
          observations: d.observacoes || '',
          contact: d.contacto || '',
          responsible: d.responsavel || '',
          date: d.created_at || new Date().toISOString(),
          fileUrl: d.file_url, // URL stored in DB
  ...
  const handleSaveDocument = async () => {
    ...
    try {
      const { uploadArquivo, supabase } = await import('../services/supabaseClient');

      let finalFileUrl = editingDoc?.fileUrl;

      // Handle File Upload if selected
      if (selectedFile) {
        const uploadData = await uploadArquivo(selectedFile);
        if (uploadData && uploadData.path) {
          // Construct Public URL (Assuming public bucket)
          const { data: { publicUrl } } = supabase.storage.from('arquivos').getPublicUrl(uploadData.path);
        ...
        <div className="fixed inset-0 z-[110] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden animate-in zoom-in-95">
            ...
            <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="col-span-2">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-2">Anexar Ficheiro</label>
                <div className="border-2 border-dashed border-slate-300 rounded-xl p-6 text-center hover:bg-slate-50 transition cursor-pointer relative">
                  ...
                  <div className="flex flex-col items-center gap-2 text-slate-400">
                    <Upload size={24} />
                    {selectedFile ? (
                      <span className="font-bold text-blue-600">{selectedFile.name}</span>
                    ) : (
                      <span className="text-xs font-bold uppercase">Clique ou arraste para carregar ficheiro</span>
                    )}
                  </div>
                </div>
              </div>
