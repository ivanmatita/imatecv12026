...
const PurchaseList: React.FC<PurchaseListProps> = ({ purchases: propPurchases, onDelete, onUpdate, onCreateNew, onUpload, onSaveSupplier }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('ALL');
  const [typeFilter, setTypeFilter] = useState('ALL');

  // Internal State for Cloud Data
  const [cloudPurchases, setCloudPurchases] = useState<Purchase[]>([]);
  const [isCloudLoading, setIsCloudLoading] = useState(false);

  // Combine props (if any) with cloud data
  const purchases = useMemo(() => {
    // Deduplicate by ID
    const combined = [...cloudPurchases, ...propPurchases];
    return Array.from(new Map(combined.map(item => [item.id, item])).values());
  }, [cloudPurchases, propPurchases]);

  useEffect(() => {
    async function fetchCloudPurchases() {
      try {
        setIsCloudLoading(true);
        const { listarCompras } = await import('../services/supabaseClient');
        const data = await listarCompras();
        if (data) {
          // Map DB data to Purchase type (Best effort mapping)
          const mapped: Purchase[] = data.map((d: any) => ({
            id: d.id,
            date: d.data || d.data_emissao || new Date().toISOString(),
            dueDate: d.data || d.data_vencimento || new Date().toISOString(),
            documentNumber: d.numero_documento || d.id.substring(0, 8),
            supplier: d.fornecedor || d.fornecedor_nome || 'Fornecedor Desconhecido',
            supplierId: d.fornecedor_id || 'unknown',
            nif: d.nif || d.fornecedor_nif || '999999999',
            supplierAddress: '',
            total: Number(d.valor || d.total || 0),
            subtotal: Number(d.valor || d.total || 0), // Simplification
            taxAmount: 0,
            status: (d.estado || d.status) === 'Pago' ? 'PAID' : 'PENDING',
            type: (d.tipo as PurchaseType) || PurchaseType.FT,
            items: [],
            currency: 'AOA',
            exchangeRate: 1,
            globalDiscount: 0,
          ...
          setCloudPurchases(mapped);
        }
      } catch (error) {
        console.error("Erro ao carregar compras da cloud:", error);
      } finally {
  ...
  const ActionButton = ({ icon: Icon, label, onClick, danger = false, disabled = false }: any) => (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`w-full text-left p-3 rounded-xl border flex items-center justify-between transition-all group disabled:opacity-50 ${danger
