...
const SecretariaList: React.FC<SecretariaListProps> = ({ documents: localDocs, onCreateNew, onEdit, onDelete }) => {
    ...
    return (
      <div className="fixed inset-0 bg-slate-800 z-[100] overflow-y-auto flex flex-col items-center animate-in fade-in duration-200">
        ...
        <div id="docPrintArea" className="bg-white shadow-2xl my-8 p-[25mm] w-[210mm] min-h-[297mm] text-black font-serif text-[11pt] leading-relaxed relative print-container">
          ...
          <div className="text-justify min-h-[100mm]" dangerouslySetInnerHTML={{ __html: String(previewDoc.corpo) }}></div>
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300 p-6 bg-slate-50 min-h-screen font-sans">
      {renderPreviewModal()}

      {/* Teramind Header */}
      <div className="bg-gradient-to-r from-orange-500 to-amber-600 p-6 rounded-t-2xl shadow-lg flex flex-col md:flex-row justify-between items-center gap-4 text-white">
        <div>
          <h1 className="text-2xl font-light flex items-center gap-2 tracking-tight">
            <FileText className="text-white/80" /> Secretaria Digital
            {isLoading && <RefreshCw size={16} className="animate-spin text-white ml-2" />}
          </h1>
          <p className="text-xs text-orange-100 font-bold uppercase tracking-widest mt-1">
            Gestão de cartas, declarações e memorandos
          </p>
        </div>
        <div className="flex gap-2">
          <button onClick={loadDocuments} className="px-3 py-2 bg-white/10 text-white rounded hover:bg-white/20 border border-white/20 text-xs font-bold flex items-center gap-2 backdrop-blur-sm transition">
            <RefreshCw size={16} /> Sincronizar
          </button>
          <button onClick={onCreateNew} className="px-4 py-2 bg-emerald-500 text-white rounded hover:bg-emerald-600 transition text-xs font-bold flex items-center gap-2 shadow-lg uppercase tracking-wider">
      ...
      {fetchError && (
        <div className="bg-red-50 text-red-700 p-4 rounded-lg border border-red-200 flex items-start gap-3 animate-in slide-in-from-top-2">
          ...
          <div>
            ...
            <p className="text-xs opacity-80">{String(fetchError)}</p>
      ...
      <div className="bg-white p-3 border-x border-slate-200 flex items-center gap-3 shadow-inner bg-slate-50/50 -mt-6">
      ...
      <div className="bg-white border-x border-b border-slate-200 rounded-b-xl overflow-hidden shadow-xl -mt-0">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-50 text-slate-500 font-bold uppercase tracking-wider border-b border-slate-200">
              <tr>
                <th className="p-4 font-bold text-slate-400">Data</th>
                <th className="p-4 font-bold text-slate-400">Número</th>
                <th className="p-4 font-bold text-slate-400">Tipo</th>
                <th className="p-4 font-bold text-slate-400">Assunto</th>
                <th className="p-4 font-bold text-slate-400">Destinatário</th>
                <th className="p-4 text-center font-bold text-slate-400">Ações</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 text-slate-700">
              {filteredDocs.map(doc => (
                <tr key={doc.id} className="hover:bg-orange-50/50 transition-colors">
                  <td className="p-4 font-medium text-slate-600">{formatDate(doc.date)}</td>
                  <td className="p-4 font-mono font-bold text-orange-600">{doc.number}</td>
                  <td className="p-4">
                    <span className="bg-slate-100 text-slate-600 px-2 py-1 rounded text-[10px] uppercase font-bold border border-slate-200">{doc.type}</span>
                  </td>
                  <td className="p-4 font-medium max-w-[300px] truncate text-slate-800">{doc.assunto}</td>
                  <td className="p-4 font-medium text-slate-600">{doc.destinatarioNome}</td>
              ...
              {filteredDocs.length === 0 && !isLoading && (
                <tr><td colSpan={6} className="p-12 text-center text-slate-400 italic bg-slate-50/50">Nenhum documento encontrado.</td></tr>
              )}
            </tbody>
          </table>
        </div>
