...
export interface Employee {
  ...
  workLocationMunicipality?: string;
  irtGroup?: string;
...
export interface DailyAttendance {
  status: 'ADMISSAO' | 'FOLGA' | 'SERVICO' | 'FALTA_JUST' | 'FALTA_INJUST' | 'FERIAS' | 'NONE';
  overtimeHours: number;
  lostHours: number;
  location: string;
  subsidyFoodValue?: number;
  subsidyTransportValue?: number;
}
...
export interface AttendanceRecord {
  ...
  month: number;
  year: number;
  days: Record<number, DailyAttendance>;
}
