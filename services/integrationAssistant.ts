        ...
        try {
            ...
            switch (modulo) {
                case 'clientes':
                    ...
                    mappedData = dados.map(d => IntegrationAssistant.mapearCliente(d));
                    break;
                case 'fornecedores':
                    dados = await BackendAssistant.fornecedores.listar();
                    mappedData = dados.map(d => IntegrationAssistant.mapearFornecedor(d));
                    break;
                case 'vendas': // Faturas
                    dados = await BackendAssistant.vendas.listar();
                    mappedData = dados.map(d => IntegrationAssistant.mapearFatura(d));
                    break;
                case 'compras':
                    dados = await BackendAssistant.compras.listar();
                    mappedData = dados.map(d => IntegrationAssistant.mapearCompra(d));
                    break;
                case 'caixas':
                    dados = await BackendAssistant.caixas.listar();
                    mappedData = dados.map(d => IntegrationAssistant.mapearCaixa(d));
                    break;
                case 'series':
                    dados = await BackendAssistant.series.listar();
                    mappedData = dados.map(d => IntegrationAssistant.mapearSerie(d));
                    break;
                case 'utilizadores':
                    dados = await BackendAssistant.utilizadores.listar();
                    mappedData = dados.map(d => IntegrationAssistant.mapearUtilizador(d));
                    break;
                case 'metricas':
                    dados = await BackendAssistant.metricas.listar();
                    mappedData = dados.map(d => IntegrationAssistant.mapearMetrica(d));
                    break;
                case 'armazens':
                    dados = await BackendAssistant.armazens.listar();
                    mappedData = dados.map(d => IntegrationAssistant.mapearArmazem(d));
                    break;
                case 'produtos':
                    dados = await BackendAssistant.produtos.listar();
                    mappedData = dados.map(d => IntegrationAssistant.mapearProduto(d));
                    break;
            }
