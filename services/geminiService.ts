...
// Carregar API Key do Gemini das variáveis de ambiente
// @ts-ignore
const GEMINI_API_KEY = (import.meta as any).env.VITE_GEMINI_API_KEY;

// Inicializar Gemini apenas se a API Key estiver disponível
...
if (GEMINI_API_KEY && GEMINI_API_KEY !== 'PLACEHOLDER_API_KEY') {
  try {
    ...
    console.warn('⚠️ Erro ao inicializar Gemini AI:', error);
    ai = null;
  }
} else {
  console.info('ℹ️ Gemini API Key não configurada. Funcionalidades de IA desabilitadas.');
}
...
export const generateInvoiceDataFromText = async (
  ...
  // Se Gemini não estiver disponível, retornar dados vazios
  if (!ai) {
    console.warn('⚠️ Gemini AI não disponível. Retornando dados vazios.');
    return {
      items: [],
      notes: 'Gemini AI não configurado. Configure VITE_GEMINI_API_KEY no arquivo .env.local'
    };
  }
  ...
  try {
    ...
    return {
      items: [],
      notes: 'Erro ao processar com Gemini AI'
    };
  }
};

export const analyzeFinancialHealth = async (invoiceSummary: string) => {
  // Se Gemini não estiver disponível, retornar mensagem padrão
  if (!ai) {
    return "Gemini AI não configurado. Para usar análises financeiras, configure VITE_GEMINI_API_KEY no arquivo .env.local";
  }
...
export const getAIBusinessAssistantResponse = async (
  ...
  // Se Gemini não estiver disponível, retornar mensagem padrão
  if (!ai) {
    return "Gemini AI não configurado. Para usar o assistente virtual, configure VITE_GEMINI_API_KEY no arquivo .env.local";
  }
